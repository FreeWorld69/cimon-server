...
export class MovieService {
  ...
  async getPopularMovies(): Promise<MovieSchema[]> {
    const { data } = await this.moviesApiService.getPopularMovies(String(MovieService.SOURCE));
  ...
  async getSeries(id: number, season: number): Promise<SeasonFileSchema[]> {
    const { data } = await this.moviesApiService.getSeasonFiles(id, season, MovieService.SOURCE);
